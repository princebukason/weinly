...
export async function middleware(request: NextRequest) {
  ...
  // Admin route — let the page itself handle unauthenticated state (shows login form)
  // Only block if a session cookie is present but invalid
  if (path.startsWith("/admin")) {
    const adminPassword = process.env.ADMIN_PASSWORD;
    const sessionToken = request.cookies.get("weinly_admin_session")?.value;

    // No session cookie → let the page load (it shows the login form)
    if (!sessionToken) {
      return NextResponse.next({ request });
    }

    // Session cookie present but ADMIN_PASSWORD not configured → clear and allow
    if (!adminPassword) {
      return NextResponse.next({ request });
    }
