      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-3">
          ...
          <div className="relative z-10 grid grid-cols-1 gap-8 md:grid-cols-2 md:items-center">
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-indigo-500/25 bg-indigo-500/10 px-4 py-1.5">
                ...
                <span className="text-xs font-semibold text-indigo-300">Pricing built for serious buyers</span>
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <div className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-xs font-semibold text-emerald-300">Faster sourcing</div>
                <div className="rounded-full border border-sky-500/20 bg-sky-500/10 px-4 py-2 text-xs font-semibold text-sky-300">Better supplier matching</div>
                <div className="rounded-full border border-violet-500/20 bg-violet-500/10 px-4 py-2 text-xs font-semibold text-violet-300">Safer buying flow</div>
              </div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 md:p-6">
              <div className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-indigo-300">Why buyers upgrade</div>
              <div className="space-y-3">
                {[
                  ...
                  <div key={item} className="flex items-start gap-3 rounded-2xl border border-white/7 bg-white/4 p-3">
        ...
        <section className="flex justify-center">
          <div className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-1.5">
            <button onClick={() => setBillingCycle("monthly")}
              className={`rounded-xl border-0 cursor-pointer px-5 py-2 text-sm font-bold transition-all ${billingCycle === "monthly" ? "bg-gradient-to-r from-indigo-500 to-indigo-700 text-white shadow-lg" : "bg-transparent text-slate-500"}`}>
              Monthly
            </button>
            <button onClick={() => setBillingCycle("yearly")}
              className={`rounded-xl border-0 cursor-pointer px-5 py-2 text-sm font-bold transition-all ${billingCycle === "yearly" ? "bg-gradient-to-r from-indigo-500 to-indigo-700 text-white shadow-lg" : "bg-transparent text-slate-500"}`}>
              Yearly
              <span className="ml-2 rounded-full bg-emerald-500/20 px-2 py-0.5 text-xs text-emerald-400">Save {yearlySavingShort}</span>
            </button>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-4">
          ...
          <div className="flex h-full flex-col rounded-3xl border border-white/7 bg-[#111827] p-6">
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Free</div>
            ...
            <a href="/#main-tabs" className="mt-6 block rounded-xl border border-white/10 bg-white/6 py-3 text-center text-sm font-bold text-slate-300 no-underline">
              Start free
            </a>
          </div>
          ...
          {/* Contact Unlock */}
          <div className="flex h-full flex-col rounded-3xl border border-white/7 bg-[#111827] p-6">
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Contact Unlock</div>
            ...
            <a href="/#main-tabs" className="mt-6 block rounded-xl border border-indigo-500/30 bg-indigo-500/10 py-3 text-center text-sm font-bold text-indigo-300 no-underline transition-all hover:bg-indigo-500/15">
              Submit a request
            </a>
          </div>
          ...
          <div className="relative flex h-full flex-col rounded-3xl border border-indigo-500/30 bg-gradient-to-b from-indigo-950 to-violet-950 p-6 shadow-2xl shadow-indigo-500/15">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2">
              <span className="rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 px-4 py-1.5 text-xs font-bold text-white whitespace-nowrap">Most popular</span>
            </div>
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-indigo-300">Weinly Pro</div>
            <div className="mb-2 flex items-end gap-2">
              <div className="text-4xl font-black tracking-tight text-white">{currentPrice}</div>
            ...
            {billingCycle === "yearly" && (
              <div className="mb-4 inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-bold text-emerald-400">
            ...
            <div className="mt-6 flex flex-col gap-2">
              <button onClick={() => handleProPayment(currentPlan)} disabled={loading !== null}
                className="w-full cursor-pointer rounded-xl border-0 bg-gradient-to-r from-indigo-500 to-violet-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/30 disabled:opacity-60">
          ...
          {/* Enterprise */}
          <div className="flex h-full flex-col rounded-3xl border border-white/7 bg-[#111827] p-6">
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Enterprise</div>
            <div className="mb-2 text-4xl font-black tracking-tight text-white">Custom</div>
            ...
            <a href={supportLink} target="_blank" rel="noreferrer" className="mt-6 block rounded-xl border border-amber-500/20 bg-amber-500/10 py-3 text-center text-sm font-bold text-amber-400 no-underline">
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {[
            ...
            <div key={t.title} className="rounded-3xl border border-white/7 bg-[#111827] p-5">
              <div className="mb-2 text-sm font-bold text-white">{t.title}</div>
              <p className="text-sm leading-relaxed text-slate-500">{t.text}</p>
            </div>
          ))}
        </section>
        ...
        <section className="rounded-3xl border border-white/7 bg-[#111827] p-6 md:p-10">
          <div className="mb-3 inline-block rounded-full bg-indigo-500/12 px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-indigo-400">FAQ</div>
          <h2 className="mb-8 text-2xl font-black tracking-tight text-white md:text-3xl">Frequently asked questions</h2>
        ...
        <section className="hidden md:block rounded-3xl border border-indigo-500/20 bg-gradient-to-r from-indigo-900/60 to-violet-900/60 p-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <button onClick={() => handleProPayment(currentPlan)} disabled={loading !== null}
              className="cursor-pointer rounded-xl border-0 bg-gradient-to-r from-indigo-500 to-violet-600 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/25 disabled:opacity-60">
